-- MySQL Script generated by MySQL Workbench
-- seg 05 abr 2021 04:34:12
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema ferrocene
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Table `group`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `group` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL,
  `name` VARCHAR(190) NOT NULL,
  `desc` MEDIUMTEXT NOT NULL DEFAULT '',
  PRIMARY KEY (`uuid`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE UNIQUE INDEX `name_UNIQUE` ON `group` (`name` ASC);


-- -----------------------------------------------------
-- Table `app`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `app` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL,
  `name` VARCHAR(255) NOT NULL,
  `url` VARCHAR(1024) NOT NULL,
  `users_group` BINARY(16) NOT NULL,
  `key` MEDIUMBLOB NOT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_app_group1`
    FOREIGN KEY (`users_group`)
    REFERENCES `group` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE UNIQUE INDEX `users_group_UNIQUE` ON `app` (`users_group` ASC);

CREATE INDEX `fk_app_group1` ON `app` (`users_group` ASC);


-- -----------------------------------------------------
-- Table `audit`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `audit` (
  `audit_id` INT(11) NOT NULL AUTO_INCREMENT,
  `ip_addr_real` VARCHAR(44) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NULL DEFAULT NULL,
  `ip_addr_peer` VARCHAR(44) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NULL DEFAULT NULL,
  `when` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `subject_uuid` BINARY(16) NULL DEFAULT NULL,
  `level` CHAR(1) NOT NULL,
  `msg_code` VARCHAR(190) NOT NULL,
  `msg` MEDIUMTEXT NOT NULL,
  `kv` MEDIUMBLOB NULL DEFAULT NULL,
  PRIMARY KEY (`audit_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE INDEX `subject_uuid_IDX` ON `audit` (`subject_uuid` ASC);

CREATE INDEX `when_IDX` ON `audit` (`when` ASC);

CREATE INDEX `ip_addr_real_IDX` ON `audit` (`ip_addr_real` ASC);

CREATE INDEX `ip_addr_peer_IDX` ON `audit` (`ip_addr_peer` ASC);


-- -----------------------------------------------------
-- Table `object_type`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `object_type` (
  `uuid` BINARY(16) NOT NULL,
  `type` VARCHAR(30) NOT NULL,
  PRIMARY KEY (`uuid`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;


-- -----------------------------------------------------
-- Table `group_members`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `group_members` (
  `group_uuid` BINARY(16) NOT NULL,
  `member_uuid` BINARY(16) NOT NULL,
  PRIMARY KEY (`group_uuid`, `member_uuid`),
  CONSTRAINT `fk_group_members_group`
    FOREIGN KEY (`group_uuid`)
    REFERENCES `group` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_group_members_object_type1`
    FOREIGN KEY (`member_uuid`)
    REFERENCES `object_type` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE INDEX `fk_group_members_object_type1_idx` ON `group_members` (`member_uuid` ASC);


-- -----------------------------------------------------
-- Table `kv`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `kv` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `object_uuid` BINARY(16) NOT NULL,
  `key` VARCHAR(190) NOT NULL,
  `val` VARCHAR(190) NULL DEFAULT NULL,
  `val_extra` MEDIUMBLOB NULL DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_kv_object_type1`
    FOREIGN KEY (`object_uuid`)
    REFERENCES `object_type` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE INDEX `uuid_IDX` ON `kv` (`object_uuid` ASC);

CREATE INDEX `key_IDX` ON `kv` (`key` ASC);

CREATE INDEX `fk_kv_object_type1` ON `kv` (`object_uuid` ASC);


-- -----------------------------------------------------
-- Table `user`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `user` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `superuser` TINYINT NOT NULL DEFAULT 0,
  `display_name` VARCHAR(30) NOT NULL,
  `added` DATETIME NOT NULL DEFAULT NOW(),
  `last_login` DATETIME NULL,
  PRIMARY KEY (`uuid`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;


-- -----------------------------------------------------
-- Table `login_handle`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `login_handle` (
  `login_handle` VARCHAR(190) NOT NULL,
  `user_uuid` BINARY(16) NOT NULL,
  `kind` VARCHAR(50) NOT NULL DEFAULT 'OTHER',
  `added` DATETIME NOT NULL DEFAULT NOW(),
  PRIMARY KEY (`login_handle`),
  CONSTRAINT `fk_login_handle_user`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE INDEX `user_uuid_IDX` ON `login_handle` (`user_uuid` ASC);

CREATE INDEX `fk_login_handle_user` ON `login_handle` (`user_uuid` ASC);


-- -----------------------------------------------------
-- Table `session`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `session` (
  `uuid` BINARY(16) NOT NULL,
  `user_uuid` BINARY(16) NOT NULL,
  `real_user_uuid` BINARY(16) NOT NULL,
  `login_time` DATETIME NOT NULL DEFAULT NOW(),
  `last_used` DATETIME NOT NULL DEFAULT NOW(),
  `remember_me` TINYINT NOT NULL DEFAULT 0,
  `ip_addr_real` VARCHAR(44) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NOT NULL,
  `ip_addr_peer` VARCHAR(44) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NOT NULL,
  `user_agent` VARCHAR(190) NOT NULL,
  `data` JSON NOT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_session_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_session_user2`
    FOREIGN KEY (`real_user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;

CREATE INDEX `fk_session_user_idx` ON `session` (`user_uuid` ASC);

CREATE INDEX `fk_session_real_user_idx` ON `session` (`real_user_uuid` ASC);


-- -----------------------------------------------------
-- Table `basic_otp`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `basic_otp` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `user_uuid` BINARY(16) NOT NULL,
  `algorithm` VARCHAR(16) NOT NULL,
  `codes` MEDIUMTEXT NOT NULL,
  `added` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `used` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_recovery_code_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_recovery_code_user1_idx` ON `basic_otp` (`user_uuid` ASC);

CREATE INDEX `fk_recovery_code_user1` ON `basic_otp` (`user_uuid` ASC);


-- -----------------------------------------------------
-- Table `auto_otp`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `auto_otp` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `user_uuid` BINARY(16) NOT NULL,
  `algorithm` VARCHAR(10) NOT NULL,
  `key` VARBINARY(500) NOT NULL,
  `counter` INT NULL DEFAULT NULL COMMENT 'NULL if TOTP',
  `added` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `last_used` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_auto_otp_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_auto_otp_user1_idx` ON `auto_otp` (`user_uuid` ASC);

CREATE INDEX `fk_auto_otp_user1` ON `auto_otp` (`user_uuid` ASC);


-- -----------------------------------------------------
-- Table `webauthn`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webauthn` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `user_uuid` BINARY(16) NOT NULL,
  `aaguid` BINARY(16) NULL DEFAULT NULL,
  `name` VARCHAR(180) NOT NULL,
  `counter` INT NOT NULL,
  `cloned` TINYINT NOT NULL DEFAULT 0,
  `allow_cloning` TINYINT NOT NULL DEFAULT 0,
  `cred_id` MEDIUMBLOB NOT NULL,
  `pub_key` MEDIUMBLOB NOT NULL,
  `added` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `last_used` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_webauthn_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_webauthn_user1_idx` ON `webauthn` (`user_uuid` ASC);

CREATE UNIQUE INDEX `name_UNIQUE` ON `webauthn` (`user_uuid` ASC, `name` ASC);

CREATE INDEX `fk_webauthn_user1` ON `webauthn` (`user_uuid` ASC);


-- -----------------------------------------------------
-- Table `scope`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `scope` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `name` VARCHAR(190) NOT NULL,
  `desc` MEDIUMTEXT NOT NULL DEFAULT '',
  `keys` MEDIUMTEXT NOT NULL DEFAULT '[]',
  PRIMARY KEY (`uuid`))
ENGINE = InnoDB;

CREATE UNIQUE INDEX `name_UNIQUE` ON `scope` (`name` ASC);


-- -----------------------------------------------------
-- Table `app_user_scope`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `app_user_scope` (
  `app_uuid` BINARY(16) NOT NULL,
  `user_uuid` BINARY(16) NOT NULL,
  `scope_uuid` BINARY(16) NOT NULL,
  PRIMARY KEY (`app_uuid`, `user_uuid`, `scope_uuid`),
  CONSTRAINT `fk_app_user_scope_app1`
    FOREIGN KEY (`app_uuid`)
    REFERENCES `app` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_app_user_scope_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_app_user_scope_scope1`
    FOREIGN KEY (`scope_uuid`)
    REFERENCES `scope` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_app_user_scope_user1_idx` ON `app_user_scope` (`user_uuid` ASC);

CREATE INDEX `fk_app_user_scope_scope1_idx` ON `app_user_scope` (`scope_uuid` ASC);

CREATE INDEX `fk_app_user_scope_user1` ON `app_user_scope` (`user_uuid` ASC);

CREATE INDEX `fk_app_user_scope_scope1` ON `app_user_scope` (`scope_uuid` ASC);


-- -----------------------------------------------------
-- Table `password`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `password` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `user_uuid` BINARY(16) NOT NULL,
  `name` VARCHAR(45) NOT NULL DEFAULT 'Main Password',
  `algorithm` VARCHAR(16) NOT NULL,
  `password` VARCHAR(512) CHARACTER SET 'ascii' COLLATE 'ascii_general_ci' NOT NULL,
  `requires_2fa` TINYINT NOT NULL DEFAULT 1,
  `added` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(),
  `last_used` DATETIME NULL DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_table1_user1`
    FOREIGN KEY (`user_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_table1_user1_idx` ON `password` (`user_uuid` ASC);

CREATE INDEX `fk_table1_user1` ON `password` (`user_uuid` ASC);

CREATE UNIQUE INDEX `name_UNIQUE` ON `password` (`user_uuid` ASC, `name` ASC);


-- -----------------------------------------------------
-- Table `history`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `history` (
  `object_uuid` BINARY(16) NOT NULL,
  `revision` INT NOT NULL,
  `content` MEDIUMBLOB NOT NULL,
  `user_uuid` BINARY(16) NULL,
  `real_user_uuid` BINARY(16) NULL,
  `session_uuid` BINARY(16) NULL,
  `when` DATETIME NOT NULL DEFAULT NOW(),
  PRIMARY KEY (`object_uuid`, `revision`),
  CONSTRAINT `fk_history_object_type1`
    FOREIGN KEY (`object_uuid`)
    REFERENCES `object_type` (`uuid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Storing data here is the duty of the application, not of the DBMS';


-- -----------------------------------------------------
-- Table `policy_rule`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `policy_rule` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `title` VARCHAR(190) NOT NULL,
  `desc` MEDIUMTEXT NOT NULL,
  `code` MEDIUMTEXT NOT NULL,
  PRIMARY KEY (`uuid`))
ENGINE = InnoDB;

CREATE UNIQUE INDEX `title_UNIQUE` ON `policy_rule` (`title` ASC);


-- -----------------------------------------------------
-- Table `policy_delegation`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `policy_delegation` (
  `uuid` BINARY(16) NOT NULL,
  `_revision` INT NOT NULL DEFAULT 1,
  `from_uuid` BINARY(16) NOT NULL,
  `to_uuid` BINARY(16) NOT NULL COMMENT 'If it points to a group, all members will recieve this delegation',
  `desc` MEDIUMTEXT NOT NULL,
  `action` VARCHAR(190) NOT NULL,
  `resource_uuid` BINARY(16) NULL,
  `resource_group` TINYINT NOT NULL DEFAULT 0 COMMENT 'If true and resource_uuid points to a group, the delegation will apply to all members of the group refered by resource_uuid',
  `granted_at` DATETIME NOT NULL DEFAULT NOW(),
  `valid_until` DATETIME NOT NULL DEFAULT NOW(),
  PRIMARY KEY (`uuid`),
  CONSTRAINT `fk_policy_delegation_user1`
    FOREIGN KEY (`from_uuid`)
    REFERENCES `user` (`uuid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `from_uuid_IDX` ON `policy_delegation` (`from_uuid` ASC);

CREATE INDEX `to_uuid_IDX` ON `policy_delegation` (`to_uuid` ASC);

CREATE INDEX `valid_until_IDX` ON `policy_delegation` (`valid_until` ASC);

CREATE INDEX `action_IDX` ON `policy_delegation` (`action` ASC);


-- -----------------------------------------------------
-- Placeholder table for view `session_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `session_view` (`uuid` INT, `user_uuid` INT, `user_display_name` INT, `real_user_uuid` INT, `real_user_display_name` INT, `login_time` INT, `last_used` INT, `remember_me` INT, `ip_addr_real` INT, `ip_addr_peer` INT, `user_agent` INT, `data` INT);

-- -----------------------------------------------------
-- Placeholder table for view `group_members_view`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `group_members_view` (`group_name` INT, `group_uuid` INT, `member_name` INT, `member_uuid` INT);

-- -----------------------------------------------------
-- View `session_view`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `session_view`;
CREATE  OR REPLACE VIEW `session_view` AS SELECT
  s_tbl.`uuid`,
  s_tbl.`user_uuid`,
  user_tbl.`display_name` AS `user_display_name`,
  s_tbl.`real_user_uuid`,
  real_user_tbl.`display_name` AS `real_user_display_name`,
  s_tbl.`login_time`,
  s_tbl.`last_used`,
  s_tbl.`remember_me`,
  s_tbl.`ip_addr_real`,
  s_tbl.`ip_addr_peer`,
  s_tbl.`user_agent`,
  s_tbl.`data` FROM `session` s_tbl
  JOIN `user` user_tbl ON (user_tbl.`uuid` = s_tbl.`user_uuid`)
  JOIN `user` real_user_tbl ON (real_user_tbl.`uuid` = s_tbl.`real_user_uuid`);

-- -----------------------------------------------------
-- View `group_members_view`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `group_members_view`;
CREATE  OR REPLACE VIEW `group_members_view` AS
SELECT
	G1.`name` AS `group_name`,
    GM.`group_uuid`,
    G2.`name` AS `member_name`,
    GM.`member_uuid`
FROM `group_members` GM
	JOIN `group` G1 ON (G1.`uuid` = GM.`group_uuid`)
	LEFT JOIN `group` G2 ON (G2.`uuid` = GM.`member_uuid`);

DELIMITER $$
CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`group_BEFORE_INSERT` BEFORE INSERT ON `group` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'GROUP');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`group_BEFORE_UPDATE` BEFORE UPDATE ON `group` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`group_BEFORE_DELETE` BEFORE DELETE ON `group` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`app_BEFORE_INSERT` BEFORE INSERT ON `app` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'APP');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`app_BEFORE_DELETE` BEFORE DELETE ON `app` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`app_BEFORE_UPDATE` BEFORE UPDATE ON `app` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`kv_BEFORE_UPDATE` BEFORE UPDATE ON `kv` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`user_BEFORE_INSERT` BEFORE INSERT ON `user` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'USER');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`user_BEFORE_UPDATE` BEFORE UPDATE ON `user` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`user_BEFORE_DELETE` BEFORE DELETE ON `user` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`session_BEFORE_INSERT` BEFORE INSERT ON `session` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'SESSION');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`session_BEFORE_DELETE` BEFORE DELETE ON `sessions` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`basic_otp_BEFORE_INSERT` BEFORE INSERT ON `basic_otp` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'B_OTP');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`basic_otp_BEFORE_UPDATE` BEFORE UPDATE ON `basic_otp` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`basic_otp_BEFORE_DELETE` BEFORE DELETE ON `basic_otp` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`auto_otp_BEFORE_INSERT` BEFORE INSERT ON `auto_otp` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'A_OTP');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`auto_otp_BEFORE_DELETE` BEFORE DELETE ON `auto_otp` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`auto_otp_BEFORE_UPDATE` BEFORE UPDATE ON `auto_otp` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`webauthn_BEFORE_INSERT` BEFORE INSERT ON `webauthn` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'WEBAUTHN');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`webauthn_BEFORE_DELETE` BEFORE DELETE ON `webauthn` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`webauthn_BEFORE_UPDATE` BEFORE UPDATE ON `webauthn` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`scope_BEFORE_INSERT` BEFORE INSERT ON `scope` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'SCOPE');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`scope_BEFORE_DELETE` BEFORE DELETE ON `scope` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`scope_BEFORE_UPDATE` BEFORE UPDATE ON `scope` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`password_BEFORE_INSERT` BEFORE INSERT ON `password` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'PASSWORD');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`password_BEFORE_UPDATE` BEFORE UPDATE ON `password` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`password_BEFORE_DELETE` BEFORE DELETE ON `password` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`policy_rule_BEFORE_INSERT` BEFORE INSERT ON `policy_rule` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'POL_RULE');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`policy_rule_BEFORE_UPDATE` BEFORE UPDATE ON `policy_rule` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`policy_rule_BEFORE_DELETE` BEFORE DELETE ON `policy_rule` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`delegation_BEFORE_INSERT` BEFORE INSERT ON `delegation` FOR EACH ROW
BEGIN
	INSERT INTO `object_type` (`uuid`, `type`) VALUES (NEW.`uuid`, 'POL_D');
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`delegation_BEFORE_UPDATE` BEFORE UPDATE ON `delegation` FOR EACH ROW
BEGIN
	IF (NEW.`_revision` != OLD.`_revision` + 1) THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = '_revision field is incorrect', MYSQL_ERRNO = 1001;
	END IF;
END$$

CREATE DEFINER = CURRENT_USER TRIGGER `ferrocene`.`delegation_BEFORE_DELETE` BEFORE DELETE ON `delegation` FOR EACH ROW
BEGIN
	DELETE FROM `object_type` WHERE `uuid` = OLD.`uuid`;
END$$


DELIMITER ;

SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;

-- -----------------------------------------------------
-- Data for table `user`
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO `user` (`uuid`, `_revision`, `superuser`, `display_name`, `added`, `last_login`) VALUES (0xD6FCB336EE52416D9AA04A0F7D59612C, 1, 1, 'Super Admin', 'NOW()', NULL);

COMMIT;


-- -----------------------------------------------------
-- Data for table `login_handle`
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO `login_handle` (`login_handle`, `user_uuid`, `kind`, `added`) VALUES ('admin', 0xD6FCB336EE52416D9AA04A0F7D59612C, 'OTHER', 'NOW()');

COMMIT;


-- -----------------------------------------------------
-- Data for table `scope`
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO `scope` (`uuid`, `_revision`, `name`, `desc`, `keys`) VALUES (0xBBFEF0381DF84BDFB39F3A447A4DECEA, DEFAULT, 'Basic', 'User ID and display name', '[\'user.uuid\', \'user.display_name\']');
INSERT INTO `scope` (`uuid`, `_revision`, `name`, `desc`, `keys`) VALUES (0x2A86C74946F540928E7CD47EA7BA3E64, DEFAULT, 'Email Address', 'Your email addresses', '[\'user.login_handle[kind=\\\'EMAIL\\\']\']');
INSERT INTO `scope` (`uuid`, `_revision`, `name`, `desc`, `keys`) VALUES (0xD940DB94C0F346708D88850A8BF0B740, DEFAULT, 'Phone Number', 'Your phone and mobile numbers', '[\'user.login_handle[kind=\\\'PHONE\\\'|kind=\\\'MOBILE\\\']\']');
INSERT INTO `scope` (`uuid`, `_revision`, `name`, `desc`, `keys`) VALUES (0xDB935E386CFD4E6D90C1CAD84F7EA2EE, DEFAULT, 'Login Names', 'All usernames except email address, phone and mobile numbers', '[\'user.login_handle[kind!=\\\'EMAIL\\\'&kind!=\\\'PHONE\\\'&kind!=\\\'MOBILE\\\']\']');

COMMIT;


-- -----------------------------------------------------
-- Data for table `password`
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO `password` (`uuid`, `_revision`, `user_uuid`, `name`, `algorithm`, `password`, `requires_2fa`, `added`, `last_used`) VALUES (0x3B2D4234E81448F0808BBBBF9B7BF5AB, DEFAULT, 0xD6FCB336EE52416D9AA04A0F7D59612C, DEFAULT, 'Argon2', '$argon2id$v=19$m=4096,t=192,p=16$c2FsdHNhbHRzYWx0$tz06nxFEeiD7jm+po95x67Ky2K/4BstPcy+7flDnKt0', 0, DEFAULT, NULL);

COMMIT;

